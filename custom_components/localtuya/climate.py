...
class LocaltuyaClimate(LocalTuyaEntity, ClimateEntity):
    ...
    async def async_set_temperature(self, **kwargs):
        ...
        if ATTR_TEMPERATURE in kwargs and self.has_config(CONF_TARGET_TEMPERATURE_DP):
            ...
            _LOGGER.debug("Set Temperature TO %s with DP %s", temperature, self._config[CONF_TARGET_TEMPERATURE_DP])
    ...
    async def async_set_fan_mode(self, fan_mode):
        ...
        if fan_mode == FAN_OFF:
            await self.async_set_hvac_mode(HVAC_MODE_OFF) #turning off fan same as all off
            return
        if self._conf_hvac_fan_mode_dp != None:
